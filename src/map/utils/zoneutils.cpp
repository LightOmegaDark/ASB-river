            ...
            for (auto zoneId : zonesOnThisProcess)
            {
                ts.schedule([normalLevelRangeMin, normalLevelRangeMax, zoneId]()
                {
                    ...
                    auto Query = fmt::format("SELECT mob_groups.zoneid, mobname, mobid, pos_rot, pos_x, pos_y, pos_z, \
                        ...
                        mob_ele_evasion.fire_eem, mob_ele_evasion.ice_eem, mob_ele_evasion.wind_eem, mob_ele_evasion.earth_eem, mob_ele_evasion.lightning_eem, mob_ele_evasion.water_eem, mob_ele_evasion.light_eem, mob_ele_evasion.dark_eem, \
                        ...
                        mob_family_system.charmable, mob_spawn_points.spawnset, COALESCE(mob_spawn_sets.maxspawns, 0), mob_groups.content_tag \
                        ...
                        INNER JOIN mob_spawn_sets ON (mob_spawn_sets.spawnsetid = mob_spawn_points.spawnset AND mob_spawn_sets.zoneid = mob_groups.zoneid) \
                    ...
                    if (ret != SQL_ERROR && sql->NumRows() != 0)
                    {
                        while (sql->NextRow() == SQL_SUCCESS)
                        {
                            const char* contentTag = (const char*)sql->GetData(87);
                            ...
                            if (zoneType != ZONE_TYPE::DUNGEON_INSTANCED)
                            {
                                ...
                                */
                                ...
                                PMob->m_spawnSet = (uint8)sql->GetUIntData(85); // Which spawnSet a specific mob belongs to.
