...
xi.job_utils.ninja.useMijinGakure = function(player, target, ability)
...
xi.job_utils.ninja.useYonin = function(player, target, ability)
...
xi.job_utils.ninja.useInnin = function(player, target, ability)
    target:delStatusEffect(xi.effect.INNIN)
    target:delStatusEffect(xi.effect.YONIN)
    target:addStatusEffect(xi.effect.INNIN, 30, 15, 300, 0, 20)
end

xi.job_utils.ninja.useSange = function(player, target, ability)
    local potency = player:getMerit(xi.merit.SANGE)-1
    player:addStatusEffect(xi.effect.SANGE, potency * 25, 0, 60)
end

xi.job_utils.ninja.useFutae = function(player, target, ability)
    target:addStatusEffect(xi.effect.FUTAE, 0, 0, 60)
end

xi.job_utils.ninja.useIssekigan = function(player, target, ability)
    target:addStatusEffect(xi.effect.ISSEKIGAN, 25, 0, 60)
end

xi.job_utils.ninja.useMikage = function(player, target, ability)
