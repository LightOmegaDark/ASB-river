...
xi.job_utils.blue_mage.useAzureLore = function(player, target, ability)
    player:addStatusEffect(xi.effect.AZURE_LORE, 1, 0, 30)
end

xi.job_utils.blue_mage.useBurstAffinity = function(player, target, ability)
    player:addStatusEffect(xi.effect.BURST_AFFINITY, 1, 0, 30)
    return xi.effect.BURST_AFFINITY
end

xi.job_utils.blue_mage.useChainAffinity = function(player, target, ability)
    player:addStatusEffect(xi.effect.CHAIN_AFFINITY, 1, 0, 30)
    return xi.effect.CHAIN_AFFINITY
end

xi.job_utils.blue_mage.useDiffusion = function(player, target, ability)
    player:addStatusEffect(xi.effect.DIFFUSION, 1, 0, 60)
    return xi.effect.DIFFUSION
end

xi.job_utils.blue_mage.useEfflux = function(player, target, ability)
    player:addStatusEffect(xi.effect.EFFLUX, 16, 1, 60)
end

xi.job_utils.blue_mage.useUnbridledWisdom = function(player, target, ability)
    target:addStatusEffect(xi.effect.UNBRIDLED_WISDOM, 16, 1, 30)
end

xi.job_utils.blue_mage.useUnbridledLearning = function(player, target, ability)
