...
spellObject.onSpellCast = function(caster, target, spell)
    ...
    if (resist > 0.0625) then
        if (target:addStatusEffect(xi.effect.FLASH, 300, 3, duration)) then -- Flash should be 300 and quickly wear off, tick value results in 1 ACC penalty at max duration.
