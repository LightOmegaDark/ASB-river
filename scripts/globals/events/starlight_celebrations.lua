        ...
        while count ~= 0 do
            ...
            if not player:hasItem(picked) then
                ...
                if
                    picked > 178 and
                    picked < 10382
                then -- checks if reward is a food item
                    reward = picked
                    count = 0
                    npcUtil.giveItem(player, reward)
                else
        ...
        if gil >= 0 then
            if playerPoint ~= 0 then
                if playerPoint == 10 then
                    ...
                    elseif
                        elapsedTime > recordTime and
                        elapsedTime <= 540
                    then -- qualifying time
                        if hasTree then
...
function xi.events.starlightCelebration.merryMakersGoblinOnFinish(player, csid, option, id)
    if csid == 4711 then
        ...
        if rnd == 1 then
            player:addKeyItem(xi.keyItem.redPresent)
            player:messageSpecial(id.text.KEYITEM_OBTAINED, xi.keyItem.redPresent)
        elseif rnd == 2 then
            player:addKeyItem(xi.keyItem.greenPresent)
            player:messageSpecial(id.text.KEYITEM_OBTAINED, xi.keyItem.greenPresent)
        else
            player:addKeyItem(xi.keyItem.bluePresent)
            player:messageSpecial(id.text.KEYITEM_OBTAINED, xi.keyItem.bluePresent)
        end
...
function xi.events.starlightCelebration.merryMakersMoogleOnTrigger(player, npc)
    ...
    local redPresent = player:hasKeyItem(xi.keyItem.redPresent)
    local greenPresent = player:hasKeyItem(xi.keyItem.greenPresent)
    local bluePresent = player:hasKeyItem(xi.keyItem.bluePresent)
...
function xi.events.starlightCelebration.merryMakersNPCDeliverOnTrigger(player, npc, id)
    ...
    if questStatus ~= 0 then
        ...
        elseif confirmed ~= 0 then
            local npcName = GetNPCByID(sender):getName()
            local eventPos = player:getCharVar("[MerryMakers]ConfirmedID")
            local eventTable = merryMakersNPCs.Receive_Event_IDs[eventPos]
            local redPresent = player:hasKeyItem(xi.keyItem.redPresent)
            local greenPresent = player:hasKeyItem(xi.keyItem.greenPresent)
            local bluePresent = player:hasKeyItem(xi.keyItem.bluePresent)
            ...
            if currentNpc ~= sender then
                ...
                if redPresent then
                    player:delKeyItem(xi.keyItem.redPresent)
                    player:messageSpecial(id.text.KEYITEM_LOST, xi.keyItem.redPresent)
                elseif greenPresent then
                    player:delKeyItem(xi.keyItem.greenPresent)
                    player:messageSpecial(id.text.KEYITEM_LOST, xi.keyItem.greenPresent)
                elseif bluePresent then
                    player:delKeyItem(xi.keyItem.bluePresent)
                    player:messageSpecial(id.text.KEYITEM_LOST, xi.keyItem.bluePresent)
                end
